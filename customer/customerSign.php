...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty($errors)) {
        $check_query = "SELECT * FROM customer WHERE mobile = ? OR email = ? OR username = ?";
    ...
    if (empty($errors)) {
        $id = uniqid();
        $insert_query = "INSERT INTO customer (id, firstname, lastname, username, password, mobile, address, gender, email, age) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $insert_stmt = mysqli_prepare($con, $insert_query);
        mysqli_stmt_bind_param($insert_stmt, "sssssisssi", $id, $firstname, $lastname, $username, $password, $mobile, $address, $gender, $email, $age);
    ...
    <div class="container flex">
        <div class="goshdc-page flex">
            <form method="post" onsubmit="return validateForm();">
                ...
                <div class="button">
                    <a href="logCustomer.php">Already Have an Account</a>
                </div>
